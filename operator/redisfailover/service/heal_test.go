...
func TestSetOldestAsMasterNewMasterError(t *testing.T) {
	...
	mr.On("MakeMaster", mock.MatchedBy(redisConnectionOptionMatcher("0.0.0.0"))).Once().Return(errors.New(""))
...
func TestSetOldestAsMaster(t *testing.T) {
	...
	mr := &mRedisService.Client{}
	mr.On("MakeMaster", mock.MatchedBy(redisConnectionOptionMatcher("0.0.0.0"))).Once().Return(nil)
...
func TestSetOldestAsMasterMultiplePodsMakeSlaveOfError(t *testing.T) {
	...
	mr := &mRedisService.Client{}
	mr.On("MakeMaster", mock.MatchedBy(redisConnectionOptionMatcher("0.0.0.0"))).Once().Return(nil)
	ipMatcher := mock.MatchedBy(redisConnectionOptionMatcher("1.1.1.1"))
	mr.On("MakeSlaveOfWithPort", ipMatcher, "0.0.0.0", "0").Once().Return(errors.New(""))
...
func TestSetOldestAsMasterMultiplePods(t *testing.T) {
	...
	mr.On("MakeMaster", mock.MatchedBy(redisConnectionOptionMatcher("0.0.0.0"))).Once().Return(nil)
	ipMatcher := mock.MatchedBy(redisConnectionOptionMatcher("1.1.1.1"))
	mr.On("MakeSlaveOfWithPort", ipMatcher, "0.0.0.0", "0").Once().Return(nil)
...
func TestSetOldestAsMasterOrdering(t *testing.T) {
	...
	ms.On("UpdatePodLabels", namespace, mock.AnythingOfType("string"), mock.Anything).Return(nil)
	mr := &mRedisService.Client{}
	mr.On("MakeMaster", mock.MatchedBy(redisConnectionOptionMatcher("1.1.1.1"))).Once().Return(nil)
	ipMatcher := mock.MatchedBy(redisConnectionOptionMatcher("0.0.0.0"))
	mr.On("MakeSlaveOfWithPort", ipMatcher, "1.1.1.1", "0").Once().Return(nil)
...
func TestSetMasterOnAllMakeMasterError(t *testing.T) {
	...
	ms.On("UpdatePodLabels", namespace, mock.AnythingOfType("string"), mock.Anything).Once().Return(nil)
	mr := &mRedisService.Client{}
	mr.On("IsMaster", mock.MatchedBy(redisConnectionOptionMatcher("0.0.0.0"))).Return(false, errors.New(""))
	healer := rfservice.NewRedisFailoverHealer(ms, mr, log.DummyLogger{})
...
func TestSetMasterOnAllMakeSlaveOfError(t *testing.T) {
	...
	ms.On("UpdatePodLabels", namespace, mock.AnythingOfType("string"), mock.Anything).Return(nil)
	mr := &mRedisService.Client{}
	mr.On("IsMaster", mock.MatchedBy(redisConnectionOptionMatcher("0.0.0.0"))).Return(true, nil)
	ipMatcher := mock.MatchedBy(redisConnectionOptionMatcher("1.1.1.1"))
	mr.On("MakeSlaveOfWithPort", ipMatcher, "0.0.0.0", "0").Once().Return(errors.New(""))
...
func TestSetExternalMasterOnAll(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			mr := &mRedisService.Client{}
			if !expectError {
				mr.On("MakeSlaveOfWithPort", mock.MatchedBy(redisConnectionOptionMatcher("0.0.0.0")), "5.5.5.5", "6379").Once().Return(nil)
				if test.errorOnMakeSlaveOf {
					expectError = true
					mr.On("MakeSlaveOfWithPort", mock.MatchedBy(redisConnectionOptionMatcher("1.1.1.1")), "5.5.5.5", "6379").Once().Return(errors.New(""))
				} else {
					mr.On("MakeSlaveOfWithPort", mock.MatchedBy(redisConnectionOptionMatcher("1.1.1.1")), "5.5.5.5", "6379").Once().Return(nil)
				}
			}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.errorOnMonitorRedis {
				errorExpected = true
				mr.On("MonitorRedisWithPort", mock.MatchedBy(sentinelConnectionOptionMatcher("0.0.0.0")), "1.1.1.1", "0", "2", "").Once().Return(errors.New(""))
			} else {
				mr.On("MonitorRedisWithPort", mock.MatchedBy(sentinelConnectionOptionMatcher("0.0.0.0")), "1.1.1.1", "0", "2", "").Once().Return(nil)
			}
...
func TestNewSentinelMonitorWithPort(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.errorOnMonitorRedis {
				errorExpected = true
				mr.On("MonitorRedisWithPort", mock.MatchedBy(sentinelConnectionOptionMatcher("0.0.0.0")), "1.1.1.1", "6379", "2", "").Once().Return(errors.New(""))
			} else {
				mr.On("MonitorRedisWithPort", mock.MatchedBy(sentinelConnectionOptionMatcher("0.0.0.0")), "1.1.1.1", "6379", "2", "").Once().Return(nil)
			}
